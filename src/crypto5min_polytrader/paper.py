...
def init_state(starting_cash: float) -> PaperState:
    return PaperState(cash=float(starting_cash), position_base=0.0, last_price=0.0, equity=float(starting_cash), trades=0)
...
def step(state: PaperState, price: float, signal_up: bool, position_fraction: float, fee_bps: float, slippage_bps: float) -> PaperState:
    price = float(price)
    ...
    if target_value > current_value + 1e-9:
        delta_value = min(target_value - current_value, state.cash)
        if delta_value > 0:
            fill = _apply_costs(price, 'buy', fee_bps, slippage_bps)
            state.cash -= delta_value
            state.position_base += delta_value / fill
            state.trades += 1
...
def backtest(
    ...
    if eq.empty:
        return {'final_equity': starting_cash, 'return_pct': 0.0, 'trades': 0, 'equity_curve': []}
